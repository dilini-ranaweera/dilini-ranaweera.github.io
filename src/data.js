export const experiences = [
  {
    title: 'Software Development Engineer',
    org: 'Amazon Web Services (AWS)',
    period: 'March 2026 – Present',
    description:
      'Supporting C++ (and Rust) devs at Amazon :)',
    image: '/images/discreteMath.jpg',
    link: 'https://rust-lang.org/',
    tags: ['Rust', 'Bazel', 'C++'],
  },
  {
    title: 'Software Development Engineer',
    org: 'Amazon Robotics',
    period: 'June 2025 – March 2026',
    description:
      'SDE in the Robotic Movement org. Worked on distributed simulation software, implementing novel hardware simulations and troubleshooting visibility dashboards. Learned lots of cool things about robotics and coordination at scale, but ultimately left to a language team on AWS because thats where my passion was :D',
    image: '/images/discreteMath.jpg',
    link: 'https://www.aboutamazon.com/news/tag/robotics',
    tags: ['Java', 'TypeScript'],
  },
  {
    title: 'Data Analytics and Engineering Intern',
    org: 'The Vanguard Group',
    period: 'June 2024 – August 2024',
    description:
      'Data Analytics intern at Vanguard under the Chief Data Analytics Office. Built an internal portfolio metrics dashboard used by analysts to recommend funds to clients.',
    image: '/images/discreteMath.jpg',
    link: 'https://investor.vanguard.com/?cmpgn=BR:PS:XX:BR:20250128:GG:CROSS:LB~Brand_VN~GG_KC~BD_PR~EVGRN_UN~Core_MT~Exact_AT~None_EX~None:NONE:NONE:NONE:KW:NavVanguardGroup&gclsrc=aw.ds&gad_source=1&gad_campaignid=8556346934&gbraid=0AAAAADyd_RXmuVz24WCcG9ut2xPznkM-5&gclid=CjwKCAjwqazPBhALEiwAOuXqdLUFmdpWBs3gHkVLMDhIKgwRw1rvU5kBQst1IqtkLC7F_3Ax3PV-kxoCRyUQAvD_BwE',
    tags: ['Python', 'Tableau'],
  },
  {
    title: 'CIS 1600 Teaching Assistant',
    org: 'University of Pennsylvania',
    period: 'August 2022 – May 2025',
    description:
      'Teaching assistant for Mathematical Foundations of Computer Science. I write homework problems and solutions, run weekly office hours, and lead recitations for 15+ students. I also help write and grade exams.',
    image: '/images/discreteMath.jpg',
    link: 'https://www.cis.upenn.edu/~cis1600/current/',
    tags: ['Teaching', 'Discrete Math'],
  },
  {
    title: 'Web Development Intern',
    org: 'SageFrog Marketing Group',
    period: 'June 2023 – August 2023',
    description:
      'Built websites in HTML, CSS, JavaScript, and PHP, and shipped React.js sites integrated with the company CMS through an internal REST API. Implemented source control and deployment pipelines for most of the agency’s client sites.',
    image: '/images/SageFrog.jpg',
    link: 'https://www.sagefrog.com/',
    tags: ['React', 'REST API', 'CI/CD'],
  },
  {
    title: 'Undergraduate Researcher',
    org: 'University of Pennsylvania, Perelman School of Medicine',
    period: 'May 2021 – May 2023',
    description:
      'Two years of biomedical imaging research across the Laboratory for Structural, Physiologic, and Functional Imaging (LSPFI) and the Center for Neuroimaging and Therapeutics. Built compressive-sensing algorithms to reduce high-field MRI scan time, manually segmented MR images from high- and low-field scanners, and trained a CNN to detect Multiple Sclerosis lesions in low-field MRI scans (PURM-funded).',
    link: 'https://www.med.upenn.edu/lspfi/',
    tags: ['Research', 'MRI', 'ML', 'CNN', 'Medical Imaging', 'Algorithms'],
  }
]

export const projects = [
  {
    title: 'Creative Coding+ (CC+)',
    role: 'Co-Founder · CFO · Content Instructor',
    description:
      'After countless hours in Berger Auditorium working on discrete-math homework, a few friends and I realized Penn lacked a pressure-free space to learn and pursue coding — and an even rarer space to talk about how hard studying CS can feel. CC+ was born as a fully open-membership club: workshops on technical skills, personal-project opportunities, and round-table events to talk honestly about life in CS. We became an official Penn club in January 2023.',
    cta: 'Visit the CC+ site',
    href: 'https://creativecoding.neocities.org/',
    image: '/CreativeCodingPlusPhotoOne.jpg',
    imageAlt: "Creative Coding+ first Instagram post",
  },
  {
    title: 'This Website',
    role: 'Personal Project',
    description:
      'The site you are on right now. Rebuilt from scratch with a modern React stack (Vite, Framer Motion), a refreshed palette, and a proper mobile layout. Deployed to GitHub Pages through a GitHub Actions workflow — so every push lands live.',
    cta: 'View source on GitHub',
    href: 'https://github.com/dilini-ranaweera/dilini-ranaweera.github.io',
    image: '/meme.jpg',
    imageAlt: 'A silly meme that doubles as a practical joke.',
  },
]
